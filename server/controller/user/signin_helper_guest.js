...
module.exports = async (req, res) => {
  try {
    ...
    const helperGuestCreated = await helper.create({ email: helperEmail, user_type: 2 });
    ...
    if (helperGuestCreated) {
      ...
      const refreshToken = jwt.sign(helperGuestInfo, process.env.REFRESH_SECRET, {
        expiresIn: '12h',
      });
      res.status(200).json({ accessToken, messeage: 'successfully signed in', data: { id: helperGuestInfo.id }});
    } else {
