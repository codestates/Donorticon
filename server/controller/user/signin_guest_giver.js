...
module.exports = async (req, res) => {
  try {
    ...
    const giverGuestCreated = await giver.create({ email: guestEmail, user_type: 1 });
    ...
    if (giverGuestCreated) {
      ...
      const refreshToken = jwt.sign(giverGuestInfo, process.env.REFRESH_SECRET, {
        expiresIn: '12h',
      });
      res.status(200).json({ accessToken, messeage: 'successfully signed in', data: { id: giverGuestInfo.id }});
    } else {
